(function ($, undefined) {
    ...
    Form.prototype = {
        ...
        btnSave_click: function () {
            ...
            controls.btnSave.on('click', function () {
                ...
                if(controls.chkConfirm[0].checked){
                    obj.Confirmed = 1; // Confirmado
                }else{
                    obj.Confirmed = 0; // No Confirmado
                }
        ...
        AplicateDateFormat: function(){
            var that = this,
                controls = that.getControls();
            var date = that.getToDay();
            var NorthwindSession = {};

            NorthwindSession.startDate = "-3d";
            NorthwindSession.daysOfWeekDisabled = [0, 6];
            console.log("Ingreso al mètodo AplicateDateFormat");
            controls.txtDateConfirmation.datepicker({
                format: 'dd-mm-yyyy',
                endDate: "current",
                startDate: NorthwindSession.startDate,
                daysOfWeekDisabled: NorthwindSession.daysOfWeekDisabled
            });
            controls.txtDateConfirmation.val(date.day + "-" +  date.month + "-" + date.year);
        },

        getToDay:  function(){
            var f = new Date();
            var day = f.getDate().toString().length == 2 ? f.getDate() : "0" + f.getDate().toString(),
                month = (f.getMonth() + 1).toString().length == 2 ? (f.getMonth() + 1) : "0" + (f.getMonth() + 1).toString(),
                year = f.getFullYear();
